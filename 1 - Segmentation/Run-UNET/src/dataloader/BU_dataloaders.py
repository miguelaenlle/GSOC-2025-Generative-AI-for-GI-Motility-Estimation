# this script is to see that the dataloader works correctly
...
class MadisonDatasetLabeled(Dataset):
    ...
    def __init__(self, segmentation_path, augment=False) -> None:
        self.image_paths = sorted(glob.glob(os.path.join(segmentation_path, '*image*.png'))) # comes already sorted
...
if __name__ == '__main__': # if you just run this dataloader script, but if you dont run it from temrinal specifically, then this script wont be auto executed fyi

    dataset = MadisonDatasetLabeled(segmentation_path='/Users/elizabethnemeti/Desktop/images-roberta',augment=False)
