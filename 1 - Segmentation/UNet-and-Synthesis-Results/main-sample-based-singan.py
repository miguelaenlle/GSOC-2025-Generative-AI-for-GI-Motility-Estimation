...
def benchmark_unet(
    ...
    python_command = f"python '{train_unet_py}' --train_dir '{training_folder}' --val_dir '{validation_folder}' --gen_model {gen_model} --synthetic_real_ratio {synthetic_real_ratio} --exp_id 'model-{gen_model}-synthetic-real-ratio-{synthetic_real_ratio}-{timestamp}' --epoch {num_epochs} --device={device} --unet_architecture={unet_architecture} --lr={lr}"
    try:
        print(f"Running command: {python_command}")
        result = subprocess.run(python_command, shell=True, check=True)
