...
for subject in os.listdir(input_dir):
    ...
    if os.path.isdir(subject_folder):  # Ensure it's a directory
        ...
        time_point_files = sorted([f for f in os.listdir(subject_folder) if f.endswith('_predicted.nii.gz')])
        ...
        output_file = os.path.join(subject_output_folder, f"{subject}_4D_reconstructed_predicted.nii.gz")
