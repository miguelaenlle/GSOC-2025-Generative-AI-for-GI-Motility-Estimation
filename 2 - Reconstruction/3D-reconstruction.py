...
# Filter files to include only predicted masks
files = [f for f in os.listdir(input_folder) if f.endswith('_predicted.png')]
data_dict = {}
...
for subject, time_data in tqdm(data_dict.items(), desc="Reconstructing subjects"):
    ...
    for time_point, slices in time_data.items():
        ...
        output_path = os.path.join(subject_output_folder, f"{subject}_time_{time_point}_reconstructed_predicted.nii.gz")
