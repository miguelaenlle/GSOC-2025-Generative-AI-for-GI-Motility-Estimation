"""
Collection of utility functions for image processing, training, and model operations in SinGAN.

Key Functions:
    - `read_image(opt)`: Loads an image from a specified path and converts it to a compatible format.
    - `denorm(x)`: Denormalizes a tensor, bringing values back to their original range.
    - `convert_image_np(inp)`: Converts a PyTorch tensor to a NumPy array for visualization.
    - `generate_noise(size, device, num_samp, type, scale)`: Generates random noise for GAN input.
    - `save_image(real_cpu, receptive_field, ncs, epoch_num, file_name)`: Saves an image with visualization overlays.
    - `adjust_scales2image(real_, opt)`: Adjusts image scales to fit SinGAN model requirements.
    - `create_reals_pyramid(real, reals, opt)`: Creates a pyramid of scaled images for multi-scale GAN training.
"""
