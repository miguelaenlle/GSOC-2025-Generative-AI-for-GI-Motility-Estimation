...
"""
Functions for resizing images with various interpolation methods.

Key Functions:
    - `imresize(im, scale, opt)`: Resizes an image by a specified scaling factor.
    - `imresize_to_shape(im, output_shape, opt)`: Resizes an image to a target output shape.
    - `denorm(x)`: Denormalizes values from [-1, 1] back to the original image range.
    - `np2torch(x, opt)`: Converts a NumPy array to a PyTorch tensor, with normalization.
    - `resize_along_dim(im, dim, weights, field_of_view)`: Resizes an image along a single dimension.
"""
